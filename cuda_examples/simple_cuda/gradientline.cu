...
int main()
{
    ...
    for(int epoch = 0; epoch<EPOCHS; epoch++)
    {
        ...
        {
            // 1. Calculate gradients for JUST THIS BATCH
        // Notice we offset the pointers: d_x + i
        gradient_descent<<<blocks_per_batch, BLOCK_SIZE>>>(d_x + i, d_y + i, d_grad_w, d_grad_b, w, b, points_per_batch);

        // 2. Reduce the batch (now d_grad_w only has batch_size elements)
        reduce_sum<<<blocks_per_batch, BLOCK_SIZE>>>(d_grad_w, d_partial, points_per_batch);
        
        // 3. Bring the single block sum back to CPU
        float h_batch_grad_w;
        cudaMemcpy(&h_batch_grad_w, d_partial, sizeof(float), cudaMemcpyDeviceToHost);
        
        // 4. Update parameters IMMEDIATELY
        w -= LEARNING_RATE * (h_batch_grad_w / batch_size);
        }
